  ...
  const unsubscribe = onSnapshot(q, 
    ...
    (error) => {
      ...
      if (error.code === 'permission-denied') {
         console.warn("Firestore Rules blocked access to users.");
      }
    }
  );
...
export const subscribeToTasks = (companyId: string, callback: (tasks: Task[]) => void) => {
  ...
  const unsubscribe = onSnapshot(q, 
    ...
    (error) => {
      ...
      if (error.code === 'permission-denied') {
         alert("Lỗi kết nối dữ liệu: Quyền truy cập bị từ chối.\nVui lòng kiểm tra lại tab 'Rules' trong Firestore Database trên Firebase Console.\n\nHãy đảm bảo Rules cho phép đọc/ghi.");
      }
    }
  );
