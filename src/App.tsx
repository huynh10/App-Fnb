...
export default function App() {
  ...
  const triggerPrint = async (data: {
    ...
    if (systemSettings.silentPrinting) {
      if (!printer) {
        alert(`Không tìm thấy máy in mặc định cho ${data.type === 'kitchen' ? 'bếp' : 'hóa đơn'}! Vui lòng kiểm tra cài đặt máy in.`);
        return;
      }

      try {
        const success = await PrinterService.printDirect(printer, content);
        if (success) return; 
      } catch (error) {
