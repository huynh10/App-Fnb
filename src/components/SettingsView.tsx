...
import { Settings, Store, Shield, Bell, Save, User, Lock, X, Printer, Layout, Wifi, Plus, Trash2, CheckCircle2, AlertCircle, Search, Bluetooth, Monitor } from 'lucide-react';
...
export const SettingsView = ({ settings, onUpdateSettings, currentUser, onResetAllTables, onChangePassword }: { 
  ...
  const [activePrinterTab, setActivePrinterTab] = useState<'lan' | 'bluetooth' | 'usb'>('lan');
  ...
  const checkAgentStatus = async () => {
    ...
    const ok = await PrinterService.isAgentRunning();
  ...
  const handleAddPrinter = () => {
    const newPrinter: PrinterConfig = {
      ...
      name: activePrinterTab === 'lan' ? 'Máy in LAN mới' : activePrinterTab === 'bluetooth' ? 'Máy in Bluetooth' : 'Máy in USB',
      connectionType: activePrinterTab,
      ipAddress: activePrinterTab === 'lan' ? '192.168.1.100' : undefined,
      port: activePrinterTab === 'lan' ? 9100 : undefined,
      type: 'bill',
      isEnabled: true,
  ...
  const [foundPrinters, setFoundPrinters] = useState<{ ip?: string; name: string; port?: number; deviceId?: string }[]>([]);

  const [scanProgress, setScanProgress] = useState(0);
  const [currentScanningIP, setCurrentScanningIP] = useState('');

  const handleScanIP = async () => {
    if (activePrinterTab === 'bluetooth') {
      handleScanBluetooth();
      return;
    }
    setScanning(true);
    setFoundPrinters([]);
    setScanProgress(0);
    setCurrentScanningIP('Đang kết nối Print Agent...');
    try {
      const results = await PrinterService.scanNetwork('192.168.1');
      setFoundPrinters(results.map(r => ({ ip: r.ip, name: r.name, port: r.port })));
    } catch (err) {
      alert(err instanceof Error ? err.message : 'Lỗi quét mạng');
    } finally {
      setScanProgress(100);
      setScanning(false);
  ...
  const handleScanBluetooth = async () => {
    if (!(navigator as any).bluetooth) {
      alert('Trình duyệt của bạn không hỗ trợ Bluetooth. Vui lòng sử dụng Chrome hoặc Edge.');
      return;
    }
    ...
    try {
      ...
      setFoundPrinters([{
        name: device.name || 'Thiết bị Bluetooth',
        deviceId: device.id
      }]);
      setScanning(false);
    } catch (error) {
  ...
  const handleAddFoundPrinter = (printer: { ip?: string; name: string; port?: number; deviceId?: string }) => {
    const newPrinter: PrinterConfig = {
      id: Math.random().toString(36).substr(2, 9),
      name: printer.name,
      connectionType: activePrinterTab,
        ...
        <div className="bg-white/5 border border-white/10 rounded-3xl p-8 space-y-6 col-span-1 md:col-span-2">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <div className="flex bg-black/20 p-1 rounded-xl">
              {(['lan', 'bluetooth', 'usb'] as const).map(tab => (
                <button
                  key={tab}
                  onClick={() => setActivePrinterTab(tab)}
                  className={cn(
                    "px-4 py-2 rounded-lg text-[10px] font-bold transition-all flex items-center gap-2",
                    activePrinterTab === tab ? "bg-emerald-500 text-black" : "text-gray-500 hover:text-white"
                  )}
                >
                  {tab === 'lan' ? <Wifi className="w-3 h-3" /> : tab === 'bluetooth' ? <Bluetooth className="w-3 h-3" /> : <Monitor className="w-3 h-3" />}
                  {tab === 'lan' ? 'LAN/WiFi' : tab === 'bluetooth' ? 'Bluetooth' : 'USB'}
                </button>
              ))}
            </div>
          </div>
          ...
          {/* Agent Status */}
          {activePrinterTab === 'lan' && (
            <div className={cn(
              "flex items-center justify-between px-4 py-3 rounded-xl text-xs font-medium border",
              agentStatus === 'online'
              ...
              <div className="flex items-center gap-2">
                <div className={cn(
                  ...
                  : agentStatus === 'offline' ? "bg-rose-400"
                  : "bg-gray-500"
                )} />
                {agentStatus === 'online' && 'Print Agent đang chạy — sẵn sàng in'}
                {agentStatus === 'offline' && 'Print Agent chưa chạy — chạy node server.js trên máy tính quầy'}
                {agentStatus === 'unknown' && 'Chưa kiểm tra Print Agent'}
              </div>
              <button
                onClick={checkAgentStatus}
                className="px-3 py-1 bg-white/10 hover:bg-white/20 rounded-lg transition-all"
              >
                Kiểm tra
              </button>
            </div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            >
              ...
              {scanning ? 'Đang tìm kiếm...' : activePrinterTab === 'lan' ? 'Quét mạng LAN' : 'Tìm thiết bị'}
            </button>
            <button 
          ...
          {/* Scanning Progress */}
          {scanning && (
            <div className="flex items-center gap-3 px-4 py-3 bg-blue-500/10 border border-blue-500/20 rounded-xl animate-in fade-in zoom-in-95">
              <div className="w-4 h-4 border-2 border-blue-500 border-t-transparent rounded-full animate-spin flex-shrink-0" />
              <p className="text-xs text-blue-400">{currentScanningIP || 'Đang quét mạng...'}</p>
            </div>
          )}
                ...
                <div key={printer.id} className="bg-white/5 border border-white/10 rounded-2xl p-6 space-y-4 relative group">
                  ...
                  <div className="flex items-center gap-4">
                    <div className={cn(
                      ...
                      {printer.connectionType === 'lan' ? <Wifi className="w-6 h-6" /> : printer.connectionType === 'bluetooth' ? <Bluetooth className="w-6 h-6" /> : <Monitor className="w-6 h-6" />}
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {printer.connectionType === 'lan' ? (
                      ...
                      <div className="col-span-2">
                        <label className="text-[10px] text-gray-500 uppercase font-bold mb-1 block">ID Thiết bị</label>
                        <input 
                          type="text"
                          disabled
                          className="w-full bg-black/20 border border-white/5 rounded-lg py-2 px-3 text-xs text-gray-500"
                          value={printer.deviceId || 'Đã kết nối'}
                        />
                      </div>
                    )}
                  </div>
